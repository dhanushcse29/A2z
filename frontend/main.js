document.addEventListener('DOMContentLoaded', function() {
  ...
  // Simple base URL - use current domain
  const BASE_URL = '';

  // Update all fetch calls to use BASE_URL
  async function fetchWithBase(url, options = {}, retries = 2) {
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        const sessionCheck = await fetchWithBase('/api/auth/me');
      ...
      if (retries > 0) {
        ...
        return fetchWithBase(url, options, retries - 1);
      }
      throw error;
    }
  }
  ...
  async function loadAnnouncements() {
    try {
      const res = await fetchWithBase('/api/announcements');
      if (!res.ok) return;
  ...
  function showChangePassword() {
    ...
    document.getElementById('change-pw-form').onsubmit = async e => {
      ...
      const res = await fetchWithBase('/api/auth/change-password', {
        method: 'POST',
        body: JSON.stringify({
  ...
  function showDashboard() {
    Promise.all([
      fetchWithBase('/api/materials').then(async r => {
        if (!r.ok) {
          const error = await r.json().catch(() => ({ message: 'Failed to fetch materials' }));
          throw new Error(error.message);
        }
        return r.json();
      }),
      fetchWithBase('/api/students').then(async r => {
        if (!r.ok) {
          const error = await r.json().catch(() => ({ message: 'Failed to fetch students' }));
          throw new Error(error.message);
        }
        return r.json();
      }),
      fetchWithBase('/api/announcements').then(async r => {
        if (!r.ok) {
          const error = await r.json().catch(() => ({ message: 'Failed to fetch announcements' }));
          throw new Error(error.message);
        }
        return r.json();
      })
      ...
      showModal(`
        <div class="dashboard">
          <h2>Error</h2>
          <p>${error.message || 'Failed to load dashboard. Please try logging in again.'}</p>
